<script lang="ts">
    ...
    $: xTicks = rangeAroundZero(
        ...domain.x,
        $display.width > 180 ? 4 : 5
    );
...
<svg bind:this={svg}>
    ...
    <g class="axis y-axis">
        {#each range(...domain.y) as lineId}
            ...
            >
                ...
                />
                {#if yTicks.indexOf(lineId) >= 0 || lineId == 0}
        ...
        {#each range(...domain.x) as lineId}
            <g class="tick" transform="translate({xScale(lineId)},0)">
                <line
                    ...
                    class:cross-axis={lineId == 0}
                />
                {#if xTicks.indexOf(lineId) >= 0 || lineId == 0}
    ...
    {#each $currentPath as crd, i}
        <circle cx={xScale(crd.x)} cy={yScale(crd.y)} r="5" class="path-crd" />
    ...
    <circle cx={xScale($gameSession.targetCoordinate.x)} cy={yScale($gameSession.targetCoordinate.y)} r="5" class="target-crd" />
</svg>
...
<style>
    ...
    circle.path-crd {
        ...
        stroke: rgb(0, 0, 0);
        stroke-opacity: 20%;
        animation: path-reveal 0.2s linear forwards;
    }
    ...
    circle.hover-crd {
        ...
        stroke: rgb(0, 0, 0);
        stroke-opacity: 0%;
        animation: hover-reveal 0.2s linear forwards;
    }
